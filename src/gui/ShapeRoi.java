...
public class ShapeRoi extends Roi {
	...
	*   The FLATNESS is an indication of the maximum deviation between the flattened and the original shape. */
	 ...
	 * {@link gui.Roi#POLYLINE} and {@link gui.Roi#POLYGON} for open and closed shapes, respectively.
	 * Conversion of shapes open and closed with more than MAXPOLY line segments will result,
	 * respectively, in {@link gui.Roi#FREELINE} and {@link gui.Roi#FREEROI} (or
	 * {@link gui.Roi#TRACED_ROI} if {@link #forceTrace} flag is <strong><code>true</code></strong>.
	 */
	 ...
	 * {@link gui.Roi#TRACED_ROI}; else, into a {@link gui.Roi#FREEROI}. */
	 ...
	 * will be of type {@link gui.Roi#ANGLE}.
	 * If <strong><code>true</code></strong> then (sub)path will be parsed into a {@link gui.Roi#ANGLE};
	 * else, into a {@link gui.Roi#POLYLINE}. */
	 ...
	 * value will be of type {@link gui.Roi#POLYLINE} or {@link gui.Roi#POLYGON}; conversion of
	 * shapes with linear segments more than this value will result in Roi objects of type
	 * {@link gui.Roi#FREELINE} or {@link gui.Roi#FREEROI} unless the average side length
	...
	public Roi[] getRois () {
		...
		if (shape instanceof Rectangle2D.Double) {
			...
			Roi r = new gui.Line((int)((Line2D.Double)shape).getX1(), (int)((Line2D.Double)shape).getY1(), (int)((Line2D.Double)shape).getX2(), (int)((Line2D.Double)shape).getY2());
			rois.add(r);
	...
	private Roi createRoi(float[] xPoints, float[] yPoints, int roiType) {
		...
		switch(roiType) {
			case Roi.LINE: roi = new gui.Line(xPoints[0],yPoints[0],xPoints[1],yPoints[1]); break;
			case Roi.RECTANGLE:
